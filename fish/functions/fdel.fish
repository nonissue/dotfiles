# find /mnt/media/local -type f -mtime +4 -delete
...
function fdel --description "Delete files older than X days." 
    ...
    set logcmd ">> /home/ops/logs/fdel.log"
	...
	if read_confirm
		...
		printf "\n"
		echo "  File deletion skipped by user" | tee $logpath
		printf "\n"
	end
    else
	...
	if read_confirm
	    ...
	    echo "  * No folders found to delete..." | tee $logpath
	    printf "\n  Finished!\n" | tee $logpath
    end
